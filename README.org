...
(use-package equake
  ...
  ;; set distinct face for Equake: very light terminal green
  ;; foreground with dark blue background, and different font (the
  ;; Martian Mono font [see https://evilmartians.com/products/martian-mono ]
  ;; is quite nice if you want to set a smallish type size and still
  ;; have it be readable; see also the Nerd Fonts version:
  ;; https://www.nerdfonts.com/font-downloads ;
  ;; https://formulae.brew.sh/cask/font-martian-mono-nerd-font ):
  :custom-face
  (equake-buffer-face
   ((t (:inherit 'default
                 :height 80
  ...
  (setq equake-available-shells
   '("shell"
     ...
     "eshell")))
#+end_src
(You can of course add the additional configuration as per the example above to any of the other use-package examples below.)

** Elpaca
You can also use an enhanced package manager, like [[https://github.com/progfolio/elpaca][elpaca]], to install directly from this git repo:
#+begin_src elisp
(use-package equake
  :ensure (:host github :repo "emacsomancer/equake")
  :config
  (advice-add #'save-buffers-kill-terminal :before-while #'equake-kill-emacs-advice)
  ;; ....
  )
...
#+begin_src elisp
(use-package equake
  :vc (:fetcher github :repo "emacsomancer/equake")
  :ensure t
  :config
  (advice-add #'save-buffers-kill-terminal :before-while #'equake-kill-emacs-advice)
  ;; ....
...
** package-vc
Or Emacs 30+'s built-in ~package-vc~ instead of vc-use-package:
#+begin_src elisp
(use-package org-daily-reflection
  :vc (equake :fetcher github :repo "emacsomancer/equake")
  :ensure t
  :config
  (advice-add #'save-buffers-kill-terminal :before-while #'equake-kill-emacs-advice)
  ;; ....
...
** Quelpa
Or [[https://framagit.org/steckerhalter/quelpa-use-package][quelpa-use-package]]:

#+BEGIN_SRC elisp
(use-package equake
  :quelpa (equake :fetcher github :repo "emacsomancer/equake")
  :ensure t
  :config
  (advice-add #'save-buffers-kill-terminal :before-while #'equake-kill-emacs-advice)
  ;; .....
